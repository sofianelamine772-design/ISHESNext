...
const filePath = '/Users/elamine/Desktop/ISHES/src/app/app/eleve/reinscription/page.tsx';
let content = fs.readFileSync(filePath, 'utf8');

// The new content will use fetchStudentCertificateDataAction and PROGRAMS_DATA
...
export default function ReinscriptionPage() {
  ...
  const handlePayment = async () => {
    ...
    try {
      setPaying(true);
      // Pour l'instant on garde le même titre. On pourrait ajouter une logique de "nextLevel" si besoin.
      const chosenTitle = activeCourse.formationTitle;
      
      const response = await fetch('/api/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          formationId: activeCourse.id, // On utilise l'ID de l'étudiant ou de la formation pour le suivi
  ...
  if (!activeCourse) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center space-y-4 text-center">
        ...
        <h2 className="text-xl font-bold text-gray-800">Aucun cursus actif trouvé</h2>
        <p className="text-gray-500 text-sm max-w-md">Nous n'avons pas trouvé de cursus en cours pour votre profil. La réinscription n'est pas possible pour le moment.</p>
      </div>
    );
  }

  const displayedTitle = activeCourse.formationTitle;
...
console.log('Successfully updated reinscription page!');
