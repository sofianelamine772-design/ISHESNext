               ...
               <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex p-1 bg-gray-100 rounded-xl">
                     <button 
                        ...
                        className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'refuse' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                     >
                        Paiements refusés ({totalRefused})
                     </button>
                     <button 
                        onClick={() => setActiveTab("tous")}
                        className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'tous' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                     >
                  ...
                  <div className="relative w-full sm:w-auto">
                    ...
                    <input 
                      ...
                      className="w-full sm:w-80 bg-gray-50 border border-gray-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-medium"
                    />
                  </div>
               </div>
                              ...
                              <tr key={payment.id} className="hover:bg-gray-50/50 transition-colors">
                                 ...
                                 <td className="px-6 py-5">
                                    {payment.status === 'refuse' ? (
                                       <span className="px-2.5 py-1 bg-red-50 text-red-600 border border-red-200 rounded-lg text-xs font-bold inline-flex items-center">
                                          Refusé
                                       </span>
                                    ) : payment.status === 'paye' ? (
      ...
      {selectedPayment && (
         <>
            ...
            <div className="fixed inset-y-0 right-0 w-[450px] bg-white shadow-2xl z-50 transform flex flex-col transition-transform duration-300 ease-in-out border-l border-gray-200">
               {/* Drawer Header */}
               <div className="h-24 bg-gradient-to-br from-indigo-900 to-slate-900 text-white flex items-start justify-between p-6 relative">
                        ...
                        {selectedPayment.status === 'refuse' && (
                           <div className="bg-red-50 border border-red-100 rounded-xl p-4 mt-2">
                              <div className="flex items-center gap-2 mb-1">
                                 ...
                                 <span className="font-bold text-red-800 text-sm">Paiement Échoué</span>
                              </div>
                              <p className="text-sm text-red-600">Motif renvoyé par Stripe : <strong className="font-bold">{selectedPayment.reason}</strong></p>
                           </div>
                        )}
                     </div>
                  </div>
                  ...
                  <div>
                     ...
                     <div className="space-y-4 relative before:absolute before:inset-y-0 before:left-[11px] before:w-px before:bg-gray-200">
                        ...
                        <div className="relative pl-8">
                           <div className={`absolute left-0 w-6 h-6 rounded-full border-4 border-white flex items-center justify-center ${selectedPayment.status === 'refuse' ? 'bg-red-500' : 'bg-green-500'}`}></div>
                           <div className="bg-white border border-gray-100 rounded-xl p-3 shadow-sm">
                              ...
                              <p className={`text-xs font-semibold ${selectedPayment.status === 'refuse' ? 'text-red-500' : 'text-green-500'}`}>
                        ...
                        {selectedPayment.history.map((hist, idx) => (
                           <div key={idx} className="relative pl-8">
                              <div className="absolute left-0 w-6 h-6 rounded-full border-4 border-white bg-green-500 flex items-center justify-center"></div>
                              <div className="bg-gray-50 border border-gray-100 rounded-xl p-3">
                                 ...
                                 <p className="text-xs font-semibold text-green-600">Complété</p>
                              </div>
                           </div>
                        ))}
                     </div>
               ...
               <div className="p-6 border-t border-gray-100 bg-gray-50 flex gap-3">
                  {selectedPayment.status === 'refuse' ? (
                     <>
                        <Button className="flex-1 bg-blue-600 hover:bg-blue-700 shadow-md font-bold rounded-xl text-sm">
                           Relancer par Email
                        </Button>
                        <Button variant="outline" className="flex-1 border-gray-300 font-bold rounded-xl text-sm hover:bg-gray-100">
                           Copier le lien
                        </Button>
                     </>
                  ) : (
