          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-10 mb-24 max-w-5xl mx-auto">
            {presentielPrograms.map((program, i) => (
              <motion.div
                ...
                className="flex flex-col relative rounded-[2.5rem] bg-white border border-gray-100 shadow-sm transition-all hover:shadow-xl hover:-translate-y-2"
              >
                <div className="p-8 sm:p-10 flex-1 flex flex-col">
                  {/* TAGS ROW */}
                  <div className="flex items-center justify-between mb-8">
                    <span className={`px-4 py-1.5 rounded-full text-xs font-black tracking-widest uppercase border ${program.tagColor}`}>
                      {program.tagText}
                    </span>
                  ...
                  {/* TITLE & DESC */}
                  <h2 className="text-3xl font-black text-[#101828] mb-4 tracking-tight">{program.title}</h2>
                  <p className="text-sm text-gray-500 leading-relaxed min-h-[60px] mb-8 font-bold">
                    {program.subtitle}
                  </p>
                  ...
                  <div className="space-y-4 mb-10 flex-1">
                    {program.features.map((feature, idx) => (
                      <div key={idx} className="flex items-start gap-3">
                        ...
                        <span className="text-base text-gray-700 font-bold">{feature}</span>
                      </div>
                    ))}
                  </div>

                  {/* PRICE & CTA */}
                  <div className="flex flex-col gap-4 mt-auto pt-8 border-t border-gray-50">
                    <div>
                      <div className="text-4xl font-black text-[#101828] tracking-tight">{program.price}</div>
                      <div className="text-xs font-black text-gray-400 uppercase tracking-widest mt-1">
                        {program.priceSub}
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3 font-black text-xs tracking-widest uppercase">
                      <Link 
                        href={program.link}
                        className="flex items-center justify-center bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 py-4 rounded-xl transition-all shadow-sm"
                      >
                        Info
                      </Link>
                      <Link 
                        href={`/inscription?plan=${program.id}`}
                        className="flex items-center justify-center bg-[#c8a96e] text-white hover:bg-[#b0935b] py-4 rounded-xl shadow-md transition-all"
                      >
                        S'inscrire
                      </Link>
                    </div>
