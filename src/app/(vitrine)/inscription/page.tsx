...
import { Suspense, useState, useEffect, useRef, type ChangeEvent, type MouseEvent } from "react";
...
function InscriptionForm() {
  ...
  const isDistanceChildPlan =
    planId === 'arabe_enfant_distance' ||
    planId === 'tajwid_enfant_distance' ||
    planId === 'tarbiya_islamiya';
  const childNeedsClassId = planId === 'presentiel-global' || isDistanceChildPlan;
  ...
  const step1BlockReason = (() => {
    if (registrationType === 'child') {
      ...
      const incompleteChild = childrenList.find((c) => {
        ...
        if (childNeedsClassId && !c.classId) return true;
        return false;
      });
      if (incompleteChild) {
        ...
        return "Sélectionnez la classe / le créneau pour chaque enfant.";
      }
      const missingChildIdentity = childrenList.some((c) => !c.prenom?.trim() || !c.nom?.trim());
      if (missingChildIdentity) {
      ...
      if (showsAdultPresentielPicker && !formData.classId) {
                ...
                {registrationType === 'child' ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {showsAdultPresentielPicker && (() => {
                      ...
                      const dayLocked = Boolean(slot) || isFemmePresentielPlan;
                      return (
                        <div className="flex flex-col gap-6 md:col-span-2">
                          {/* Jour select */}
                          <div className="space-y-2">
              ...
              <div className="bg-gray-50 rounded-3xl p-8 border border-gray-100 max-w-xl mx-auto mb-10">
                ...
                {registrationType === 'child' && (
                  <div className="border-t border-gray-200/50 pt-4">
                      ...
                      {childrenList.map((child, idx) => (
                        <div key={idx} className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-ishes-blue/10 text-ishes-blue flex items-center justify-center font-black text-xs uppercase">
                    ...
                    {siblingDiscount > 0 && (
                      <p className="text-xs font-bold text-ishes-blue text-center mt-4">
                        Réduction fratrie : −{siblingDiscount} € ({childrenList.length - 1} enfant{childrenList.length > 2 ? 's' : ''} supplémentaire{childrenList.length > 2 ? 's' : ''})
                      </p>
                    )}
                  </div>
                )}
